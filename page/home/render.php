...
<?php echo renderQ(); ?>
<?php if (getSetState() == READ_ONLY) echo ('<input type="text" name="name" placeholder="Zadej svoje jméno:">'); ?>
<?php //echo '<pre>'.print_r(getCurrentQ(), 1).getSetHash().</pre>; ?>
	</pre>
	<input type="submit" name="submit-prev<?php if (getSetState() == READ_ONLY) echo ('-ro') ?>" value="Předchozí" <?php if (getPosition() == 0) echo 'disabled '; ?>/>
<?php if (getPosition() < getQCount()-1): ?>
	<input type="submit" name="submit-next<?php if (getSetState() == READ_ONLY) echo ('-ro') ?>" value="Další" />
<?php else: ?>
	<input type="submit" name="submit-save<?php if (getSetState() == READ_ONLY) echo ('-ro') ?>" value="<?php if (getSetState() == READ_ONLY) echo ('Konec'); else echo ('Uložit'); ?>" />
<?php endif; ?>
